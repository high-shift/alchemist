...
export interface IAMPResources {
    producer: boolean;
    consumer: boolean;
}
...
class RabbitMQClient extends EventEmitter {
    ...
    public async init() {
        try {
            ...
            console.log(error);
            throw new VError(error, 'Failed to connect to RabbitMQ');
        }
    }
